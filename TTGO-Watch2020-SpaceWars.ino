       ...
       while((digitalRead(TP_INT) == HIGH))
{
  ...
  ttgo->tft->drawLine(0,125,240,125,lightblue);
   ...
   ttgo->tft->pushImage(49, 131,  48, 48, buttons[0]);
    ...
    ttgo->tft->pushImage(0, 155,  48, 48, buttons[3]); 
     ttgo->tft->pushImage(98, 155,  48, 48, buttons[2]); 
...
if(fase==1){
  ...
  if (sensor->getAccel(acc)) {
    constexpr int kAccThreshold = 150;
    down = acc.y > kAccThreshold;
    up = acc.y < -kAccThreshold;
    right = acc.x > kAccThreshold;
    left = acc.x < -kAccThreshold;
  }
